...
class Welcome extends CoreController {
    ...
    public function doGenM($name = '') {
		is_object($this->db) || $this->database();
		...
		$baseClassContent=file_get_contents(FRAMEWORK_CORE_PATH.'/ModelTemplate/base.txt');
		$modelClassContent=file_get_contents(FRAMEWORK_CORE_PATH.'/ModelTemplate/model.txt');
		$modelClassContent=str_replace('{$}',$system['model_file_suffix'],$modelClassContent);
		foreach($tables as $table){
			$mdlFile=$system['model_folder'].'/base/'.$table.'BaseModel'.$system['model_file_suffix'];
			file_put_contents($mdlFile, str_replace('{#}',$table,$baseClassContent)) or die('<span style="color:red">写入文件失败！</span>请检查文件夹“'.$system['model_folder'].'/base”是否具有可写权限。');
			...
			file_put_contents($mdlFile, str_replace('{#}',$table,$modelClassContent));
