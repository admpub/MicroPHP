...
// cached query if it exists
        if ($this->cache_on == TRUE AND stristr($sql, 'SELECT')) {
            if ($this->_cache_init()) {
                $this->load_rdriver();
                if (NULL !== ($cache = $this->CACHE->get($sql)) && $cache !== FALSE) {
                    return $cache;
                }
            }
        }
    ...
    function load_rdriver() {
        ...
        if (!class_exists($driver, FALSE)) {
			if(defined('BASEPATH')){
				...
				echo 'Does not support database driver: ',$this->dbdriver;
				exit();
			}
        }
        return $driver;
...
// Is there a cached result?
     ...
     * @return        void
     */
    function cache_delete($segment_one = '', $segment_two = '', $segment_three = '') {
        if (!$this->_cache_init()) {
            return FALSE;
        }
        return $this->CACHE->delete($segment_one, $segment_two, $segment_three);
    }
    /**
    ...
    function _cache_init() {
		if (is_object($this->CACHE)) return TRUE;
        if (class_exists('CI_DB_Cache', false)) {
            $this->CACHE = CI_DB_Cache::getInstance(CoreLoader::$system['cache_config']['storage'], CoreLoader::$system['cache_config']);
        }
        return TRUE;
    }
