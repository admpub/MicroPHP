...
if(!defined('PHP_GETTEXT_PATH')){
  define('PHP_GETTEXT_PATH', dirname(__FILE__).'/');
}
require(PHP_GETTEXT_PATH.'streams.php');
require(PHP_GETTEXT_PATH.'gettext.php');
...
class domain {
  ...
  //static:Variables
  static public $text_domains = array();
  static public $default_domain = 'messages';
  static public $LC_CATEGORIES = array('LC_CTYPE', 'LC_NUMERIC', 'LC_TIME', 'LC_COLLATE', 'LC_MONETARY', 'LC_MESSAGES', 'LC_ALL');
  static public $EMULATEGETTEXT = 0;
  static public $CURRENTLOCALE = '';
}
...
function _get_reader($domain=null, $category=5, $enable_cache=true) {
    if (!isset($domain)) $domain = domain::$default_domain;
...
function _get_codeset($domain=null) {
    if (!isset($domain)) $domain = domain::$default_domain;
    return (isset(domain::$text_domains[$domain]->codeset))? domain::$text_domains[$domain]->codeset : ini_get('mbstring.internal_encoding');
}
...
function _bindtextdomain($domain, $path) {
    ...
    domain::$text_domains[$domain]->path = $path;
}
...
function _bind_textdomain_codeset($domain, $codeset) {
    domain::$text_domains[$domain]->codeset = $codeset;
}
