...
class CoreFn {
	...
	function set_cookie($key, $value, $life = null, $path = "/", $domian = null, $http_only = false) {
		return CoreLoader::setCookie($key, $value, $life, $path, $domian, $http_only);
	}
	...
	public static function server($key = null, $default = null) {
		return CoreInput::server($key, $default);
	}
	public static function session($key = null, $default = null) {
		return CoreInput::session($key, $default);
	}
	public static function get_rule($rule, $key, $default = null) {
		return CoreInput::get_rule($rule, $key, $default);
	}
	public static function post_rule($rule, $key, $default = null) {
		return CoreInput::post_rule($rule, $key, $default);
	}
	...
	public static function post_get_rule($rule, $key, $default = null) {
		return CoreInput::post_get_rule($rule, $key, $default);
	}
	public static function get($key = null, $default = null, $xss_clean = false) {
		return CoreInput::get($key, $default, $xss_clean);
	}
	public static function post($key = null, $default = null, $xss_clean = false) {
		return CoreInput::post($key, $default, $xss_clean);
	}
	public static function cookie($key = null, $default = null, $xss_clean = false) {
		return CoreInput::cookie($key, $default, $xss_clean);
	}
	public static function cookiRaw($key = null, $default = null, $xss_clean = false) {
		return CoreInput::cookiRaw($key, $default, $xss_clean);
	}
	public static function get_post($key = null, $default = null, $xss_clean = false) {
		return CoreInput::get_post($key, $default, $xss_clean);
	}
	...
	public static function get_int($key, $min = null, $max = null, $default = null) {
		return CoreInput::get_int($key, $min, $max, $default);
	}
	public static function post_int($key, $min = null, $max = null, $default = null) {
		return CoreInput::post_int($key, $min, $max, $default);
	}
	public static function get_post_int($key, $min = null, $max = null, $default = null) {
		return CoreInput::get_post_int($key, $min, $max, $default);
	}
	 ...
	 * @param type $_
	 */
	public static function dump($arg, $_ = null) {
		$args = func_get_args();
		if (CoreInput::isCli()) {
	 ...
	 * @return CoreTableModel
	 */
	public static function table($table_name, $db = null) {
		return CoreTableModel::M($table_name, $db);
	}
	...
	public static function url() {
		...
		if (empty(CoreLoader::$system['url_rewrite'])) {
			// url是否包含入口文件名称检查
			$self_name = stripos($action, '#') === 0 || stripos($action, '#') === 1 ? pathinfo(CoreInput::server('php_self'), PATHINFO_BASENAME) : '';
			$app_start = '?';
			$get_start = '&';
		} else {
			$self_name = '';
		...
		// 是否使用相对路经检查
		$path = (stripos($action, '?') === 0 || stripos($action, '?') === 1 ? '' : self::urlPath() . '/');
...
只能在web访问时使用，在命令行下面会抛出异常。
	 ...
	 * @return type
	 */
	public static function path($subpath = null) {
		$path = str_replace(array('/', '\\'), '/', realpath('.') . ($subpath ? '/' . trim($subpath, '/\\') : ''));
		return self::truepath($path);
	}

	/**
	 * 获取系统配置信息,也就是CoreLoader::$system里面的信息
	 *
	 * @param type $key CoreLoader::$system的键
	 * @return null
	 */

	public static function systemInfo($key = null) {
		if (is_null($key)) {
			return CoreLoader::$system;
		} elseif (isset(CoreLoader::$system[$key])) {
			return CoreLoader::$system[$key];
		} else {
			return null;
		}
	}
	 ...
	 * 获取系统数据库配置信息
	 *
	 * @param type $group 数据库组名称，即CoreLoader::$system['db']的键.
	 *                      为null时返回默认的配置组,即CoreLoader::$system['db']['active_group']指定的组。
	 * @param type $key 配置组的键,指定了key可以获取指定组的键对应的值
	 * @return null
	 */

	public static function dbInfo($group = null, $key = null) {
		if (is_null($group)) {
			$cfg = CoreLoader::$system['db'][CoreLoader::$system['db']['active_group']];
			if (is_null($key)) {
				return $cfg;
			} else {
				return isset($cfg[$key]) ? $cfg[$key] : null;
			}
		} elseif (isset(CoreLoader::$system['db'][$group])) {
			$cfg = CoreLoader::$system['db'][$group];
			if (is_null($key)) {
				return $cfg;
			} else {
				return isset($cfg[$key]) ? $cfg[$key] : null;
	...
	public static function trigger404($msg = '<h1>Not Found</h1>') {
	...
	public static function trigger500($msg = '<h1>Server Error</h1>') {
		$system = CoreLoader::$system;
		if (!headers_sent()) {
	...
	public static function core_exception_handler($exception) {
			...
			if (!empty($handle)) {
				if (is_array($handle)) {
					...
					if (method_exists($rclass_obj, $method)) {
						$rclass_obj -> {
							$method}
						($errno, $errstr, $errfile, $errline, self::get_strace());
					}
				} else {
					if (function_exists($handle)) {
						$handle($errno, $errstr, $errfile, $errline, self::get_strace());
					}
				}
			}
		}
		if ($system['debug']) {
			// @ob_clean();
			self::trigger500('<pre>' . self::format_error($errno, $errstr, $errfile, $errline) . '</pre>');
		}
		exit;
	}
...
该函数会接受所有类型的错误，应该过滤掉致命错误
	 ...
	 * @return type
	 */
	public static function core_error_handler($errno, $errstr, $errfile, $errline) {
		if (!error_reporting()) {
			return;
		}
			...
			if (!empty($handle)) {
				if (is_array($handle)) {
					...
					if (method_exists($rclass_obj, $method)) {
						$rclass_obj -> {
							$method}
						($errno, $errstr, $errfile, $errline, get_strace());
					}
				} else {
					if (function_exists($handle)) {
...
该函数会接受所有类型的错误，应该只处理致命错误
	 ...
	 * @return type
	 */
	public static function core_fatal_handler() {
		$system = CoreLoader::$system;
	...
	public static function core_db_error_handler($error) {
		$msg = '';
		if (is_array($error)) {
			foreach ($error as $m) {
				$msg .= $m . "\n";
		...
		if ($core_db[$core_db['active_group']]['db_debug'] && $system['debug']) {
	...
	public static function get_strace($is_db = false) {
		$trace = debug_backtrace(false);
		foreach ($trace as $t) {
			if (!in_array($t['function'], array('display_error', 'core_db_error_handler', 'core_fatal_handler', 'core_error_handler', 'core_exception_handler'))) {
				array_shift($trace);
			} else {
				array_shift($trace);
				break;
	...
	public static function stripslashes_all() {
	...
	public static function stripslashes2($var) {
		...
		if (is_array($var)) {
			foreach ($var as $key => $val) {
				if (is_array($val)) {
					$var[$key] = self::stripslashes2($val);
				} else {
...
Generates headers that force a download to happen
	 ...
	 * @return array
	 */
	public static function getRsCol($rows, $col_name) {
		$ret = array();
		foreach ($rows as &$row) {
			$ret[] = $row[$col_name];
		}
	 ...
	 * @return type
	 */
	public static function chRsKey($rs, $col) {
		$_rs = array();
		foreach ($rs as $v) {
			$_rs[$v[$col]] = $v;
		}
	 ...
	 * @return array
	 */
	public static function sortRs($rows, $key, $order = 'asc') {
		$sort = array();
	 ...
	 * @return array
	 */
	public static function mergeRs() {
		$argv = func_get_args();
		$argc = count($argv);
		$ret = array();
	...
	public static function enableSelectDefault($return = false) {
	...
	public static function dbCachePrefix($segment_one = '', $segment_two = '', $segment_three = '') {
		if ($segment_one == '') {
			$segment_one = CoreInput::module_name();
		}
		if ($segment_two == '') {
			$segment_two = CoreInput::controller_name();
		}
		if ($segment_three == '') {
			$segment_three = CoreInput::method_name();
		}
		$key = '/' . $segment_one . '/' . $segment_two . '/' . $segment_three . '/';
		return $key;
	}
}

if(!class_exists('Fn',false)){
	eval('class Fn extends CoreFn{}');
}
/*
echo '<pre>';
foreach (array('set_cookie' => 'setCookie', 'set_cookie_raw' => 'setCookieRaw') as $func => $true) {
	echo('function ' . $func . '($key, $value, $life = null, $path = "/", $domian = null, $http_only = false) {
					 return CoreLoader::' . $true . '($key, $value, $life, $path, $domian, $http_only);
		 }
		 ');
}
foreach (array('server', 'session') as $func) {
	echo('public static function ' . $func . '($key = null, $default = null) {
					 return CoreInput::' . $func . '($key, $default);
		 }
		 ');
}
foreach (array('get_rule', 'post_rule', 'get_post_rule', 'post_get_rule') as $func) {
	echo('public static function ' . $func . '($rule, $key, $default = null) {
...
foreach (array('get', 'post', 'cookie', 'cookie_raw', 'get_post', 'post_get') as $func) {
	...
	echo('public static function ' . $func . '($key = null, $default = null, $xss_clean = false) {
...
foreach (array('get_int', 'post_int', 'get_post_int', 'post_get_int',
	...
	echo('public static function ' . $func . '($key, $min = null, $max = null, $default = null) {
