...
if (!function_exists('core_exception_handler')) {

    function core_exception_handler($exception) {
...
if (!function_exists('core_error_handler')) {
...
if (!function_exists('core_fatal_handler')) {
...
if (!function_exists('core_db_error_handler')) {

    function core_db_error_handler($error) {
        $msg = '';
        if (is_array($error)) {
            foreach ($error as $m) {
                $msg.=$m . "\n";
...
class CoreLoader {
    ...
    public function registerErrorHandle() {
        ...
        if ($system['error_manage'] || $system['log_error']) {
            ...
            register_shutdown_function('core_fatal_handler');
        }
    }
        ...
        foreach ($library_folders as $key => $library_folder) {
            ...
            if (in_array($alias_name, array_keys(CoreLibLoader::$lib_files))) {
                ...
                foreach (CoreLibLoader::$lib_files as $aname => $obj) {
                    if (strtolower(get_class($obj)) === strtolower($classname)) {
                        return CoreLibLoader::$lib_files[$alias_name] = CoreLibLoader::$lib_files[$aname];
                    }
                }
            }
            if (file_exists($filepath)) {
                ...
                if (class_exists($classname, FALSE)) {
                    if ($new) {
                        return CoreLibLoader::$lib_files[$alias_name] = new $classname();
                    } else {
                        return null;
                    }
                } else {
        ...
        foreach ($model_folders as $key => $model_folder) {
            ...
            if (in_array($alias_name, array_keys(CoreModelLoader::$model_files))) {
                ...
                foreach (CoreModelLoader::$model_files as &$obj) {
                    if (strtolower(get_class($obj)) == strtolower($classname)) {
                        return CoreModelLoader::$model_files[$alias_name] = $obj;
                    }
                }
            }
            if (file_exists($filepath)) {
    ...
    public static function classAutoloadRegister() {
        ...
        if (is_array($auto_functions)) {
            foreach ($auto_functions as $func) {
                if (is_array($func) && $func[0] == 'CoreLoader' && $func[1] == 'classAutoloader') {
                    $found = TRUE;
                    break;
                }
            }
        ...
        if (!$found) {
            //最后注册我们的自动加载器
            spl_autoload_register(array('CoreLoader', 'classAutoloader'));
        }
    }
    ...
    public static function classAutoloader($clazzName) {
        ...
        foreach ($library_folders as $library_folder) {
            $library = $library_folder . DIRECTORY_SEPARATOR . $clazzName . $system['library_file_suffix'];
     ...
     * 注意：
     * 1.自定义Loader必须继承CoreLoader。
     * 2.一个最简单的Loader示意：(假设文件名称是：MyLoader.class.php)
     * class MyLoader extends CoreLoader {
     *      public function __construct() {
     *          parent::__construct();
     *      }
     *  }
    ...
    public static function checkUserLoader() {
        global $system;
        if (!class_exists('CoreLoaderPlus', FALSE)) {
            if (!empty($system['my_loader'])) {
                ...
                if (class_exists($clazz, FALSE)) {
                    eval('class CoreLoaderPlus extends ' . $clazz . '{}');
                } else {
                    eval('class CoreLoaderPlus extends CoreLoader{}');
                }
            } else {
                eval('class CoreLoaderPlus extends CoreLoader{}');
            }
    ...
    public static function readData(Array $map, $source_data = null) {
        $data = array();
        $formdata = is_null($source_data) ? CoreInput::post() : $source_data;
    ...
    private static function checkRule($_rule, $val, $data, $db = null) {
        if (!$db) {
            $db = CoreLoader::instance()->database();
        }
        $matches = self::getCheckRuleInfo($_rule);
        $_rule = $matches[1];
        $args = $matches[2];
        switch ($_rule) {
            ...
            case 'unique':#比如unique[user.name] , unique[user.name,id:1]
                ...
                if (isset($args[1])) {
                    ...
                    $id_col = $_id_info[0];
                    $id = $_id_info[1];
                    $id = stripos($id, '#') === 0 ? CoreInput::get_post(substr($id, 1)) : $id;
...
class CoreModelLoader {
    ...
    function __get($classname) {
        if (isset(self::$model_files[$classname])) {
            ...
            return CoreLoader::model($classname);
        }
    }
 ...
 * @property CoreInput          $input
 */
class CoreModel extends CoreLoaderPlus {
...
/**
 * Description of CoreTableModel
 *
 * @author pm
 */
class CoreTableModel extends CoreModel {
    ...
    public static function M($table, $db = null) {
        if (!isset(self::$models[$table])) {
            self::$models[$table] = new CoreTableModel();
 ...
 * CI_DB_mysql_result -> Core_DB_result -> CI_DB_result
 */
